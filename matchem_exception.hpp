#ifndef matchem_exception_hpp
#define matchem_exception_hpp
...
#ifdef MATCHEM_ATTACH
    std::cout << "Error: " << what() << std::endl;
    std::cout << "Attach via command: 'gdb -p " << getpid() << "'" << std::endl;
    std::cout << "  (you may need to use sudo)" << std::endl;
    std::cout << "  (you may also need to switch from GUI threads to the matchem cxx thread)" << std::endl;
...
////////////////////////////////////////////////////////////////////////////////
class MatchemException : public std::exception
////////////////////////////////////////////////////////////////////////////////
{
 public:
  MatchemException(const std::string& msg) : m_message(msg) {}

  virtual ~MatchemException() throw() {}
 ...
 */
#ifndef NDEBUG
#define my_assert(expr, message) if (!static_cast<bool>(expr)) { throw matchem::AssertException(__LINE__, __FILE__, std::string("For object: ") + obj_to_str(*this) + "\n" + message); }
#define my_static_assert(expr, message) if (!static_cast<bool>(expr)) { throw matchem::AssertException(__LINE__, __FILE__, message); }
#else
#define my_assert(expr, message)
#define my_static_assert(expr, message)
#endif
#define my_require(expr, message) if (!static_cast<bool>(expr)) { throw matchem::MatchemException(message); }
